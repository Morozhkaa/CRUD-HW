...
func (a *Adapter) createUser(ctx *gin.Context) {
	...
	err = a.userSvc.CreateUser(ctx, user)
	if err != nil {
...
func (a *Adapter) deleteUser(ctx *gin.Context) {
	...
	err = a.userSvc.DeleteUser(ctx, username)
	if err != nil {
		a.ErrorHandler(ctx, err)
		return
	}
...
func (a *Adapter) updateUser(ctx *gin.Context) {
	...
	err = a.userSvc.UpdateUser(ctx, username, user)
	if err != nil {
		a.ErrorHandler(ctx, err)
		return
	}
...
func (a *Adapter) getUser(ctx *gin.Context) {
	...
	user, err := a.userSvc.GetUser(ctx, username)
	if err != nil {
...
// @Param user body models.UserAuth true "userAuth"
// @Success 200 {object} models.LoginResponse "The user has been successfully authenticated."
// @Header 200 {string} Authorization "Bearer <access_token>;<refresh_token>"
// @Failure 400 {object} models.ErrorResponse "Missing required 'userAuth' parameter / token generation error."
...
func (a *Adapter) login(ctx *gin.Context) {
	...
	accessToken, refreshToken, err := a.userSvc.Login(ctx, user.Login, user.Password)
	if err != nil {
		a.ErrorHandler(ctx, err)
		return
	}
	ctx.Header("Authorization", "Bearer "+fmt.Sprintf("%s;%s", accessToken, refreshToken))
...
// @Success 200 {object} models.VerifyResponse "User successfully verified."
// @Failure 400 {object} models.ErrorResponse "Token generation error."
// @Failure 403 {object} models.ErrorResponse "Missing required 'Authorization' header / tokens expired."
// @Router /verify [post]
func (a *Adapter) verify(ctx *gin.Context) {
	...
	resp, err := a.userSvc.Verify(ctx, access, refresh)
	if err != nil {
		a.ErrorHandler(ctx, err)
		return
	}
